...
export class OrderService {
  ...
  async createOrder(createOrderDto: CreateOrderDto, customerInfor: JwtPayloadCustomer) {
    if (
      ...
      createOrderDto.cartItems.length === 0
    ) {
      throw new BadRequestException('Product details cannot be empty');
    }

    const order = await this.orderRepository.createOrder(createOrderDto, customerInfor);
    return order;
  }
  ...
  async createPaymentLink(createPaymentLinkDto: CreatePaymentLinkDto) {
    ...
    const paymentLink = await this.payosService.createPaymentLink({
      ...
      returnUrl:
        returnUrl ||
        defaultReturnUrl ||
        `http://localhost:3103/v1/customer/order/payment/return?orderId=${orderId}`,
      cancelUrl:
        cancelUrl ||
        defaultCancelUrl ||
        `http://localhost:3103/v1/customer/order/payment/cancel?orderId=${orderId}`,
    });
  ...
  async handlePaymentWebhook(webhookData: any) {
    try {
      ...
      const { orderCode, status } = (verifiedData as any)?.payment ?? {
        orderCode: 0,
        status: 'pending',
      };
      ...
      if (!order) {
        throw new NotFoundException(
          `Order with payment code ${orderCode} not found`,
        );
      }
  ...
  async handlePaymentReturn(orderId: number) {
    ...
    try {
      const paymentRequest = await this.payosService.getPaymentRequest(
        order.id,
      );
